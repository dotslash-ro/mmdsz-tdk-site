...
type SignupWorkshopProps = {
  ...
  canSignUp: boolean;
  fetchApplicationsNumber: () => Promise<void>;
};

const SignupWorkshop = ({ id, email, canSignUp, fetchApplicationsNumber }: SignupWorkshopProps) => {
  ...
  async function fetchWorkshopData() {
    ...
    if (email) {
      const resp2 = await fetch(`${workshopServerUrl}/application/has-applied`, {
        method: "POST",
  ...
  return (
    <div className="">
      <h3 className="text-md font-semibold text-gray-500">{workshop.speakers}:</h3>
      ...
      {workshop.id !== "clhf5d7jm002cmc19dsfy0491" && workshop.id != "clhf5d7li002mmc19w5zr673h" ? (
        <p className="whitespace-pre-line pb-6 pt-4 text-gray-700"> {workshop.description}</p>
      ) : (
        <p className="whitespace-pre-line pb-6 pt-4 font-semibold text-rose-700">{workshop.description}</p>
      )}
      <div className="pt-3">
